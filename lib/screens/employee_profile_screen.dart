...
class EmployeeProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // મોબાઈલ માટે Sidebar ને Drawer માં સેટ કર્યું
      drawer: const Drawer(
        child: SidebarWidget(),
      ),
      body: SafeArea(
        child: Column(
          children: [
            const TopBarWidget(), // ટોપ બાર (મેનૂ બટન સાથે)
            Expanded(
              child: SingleChildScrollView(
                padding: const EdgeInsets.all(16), // મોબાઈલ મુજબ પેડિંગ ઓછું કર્યું
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Employees > Profile", style: TextStyle(color: Colors.grey, fontSize: 12)),
                    const SizedBox(height: 8),
                    // મોબાઈલમાં બટન્સ કપાય નહીં તે માટે Wrap નો ઉપયોગ
                    Wrap(
                    ...
                    const DashboardGrid(), // હવે આ બધું એકની નીચે એક આવશે
                  ],
                ),
              ),
            ),
...
// --- DASHBOARD GRID (MOBILE VIEW - STACKED VERTICALLY) ---
...
// https://employee-attendance-trac-f4684.web.app
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        const Padding(padding: EdgeInsets.only(top: 8), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Jan", style: TextStyle(fontSize: 10, color: Colors.grey)), Text("Jun", style: TextStyle(fontSize: 10, color: Colors.grey))])),
      ],
    );
  }
  Widget _dot(Color c, String t) => Row(mainAxisSize: MainAxisSize.min, children: [CircleAvatar(radius: 4, backgroundColor: c), const SizedBox(width: 4), Text(t, style: const TextStyle(fontSize: 10, color: Colors.grey))]);
}
...
class DonutPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    drawArc(0.92, Colors.green);   // On Time
    drawArc(0.05, Colors.orange);  // Late
    drawArc(0.03, Colors.red);     // Absent
    drawArc(0.02, Colors.blue);    // Leave
  }
...
class ActiveProjectsTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        const Row(children: [Expanded(flex: 3, child: Text("Project Name", style: TextStyle(color: Colors.grey, fontSize: 11))), Expanded(flex: 2, child: Text("Role", style: TextStyle(color: Colors.grey, fontSize: 11))), Expanded(flex: 2, child: Text("Status", style: TextStyle(color: Colors.grey, fontSize: 11))), Expanded(flex: 2, child: Text("Progress", style: TextStyle(color: Colors.grey, fontSize: 11)))]),
        const Divider(),
...
class TeamMembersWidget extends StatelessWidget {
  ...
  Widget _member(String n, String p) => ListTile(dense: true, contentPadding: EdgeInsets.zero,
      leading: const CircleAvatar(
        radius: 15,
        backgroundImage: AssetImage('assets/images/f3.jpg'),
...
class UpcomingTimeOffWidget extends StatelessWidget {
  ...
  Widget _timeOff(String t, String d, String s, Color c) => Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: c.withOpacity(0.05), borderRadius: BorderRadius.circular(8), border: Border.all(color: c.withOpacity(0.1))), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(t, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12)), Text(d, style: const TextStyle(fontSize: 10, color: Colors.grey))]), Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(4)), child: Text(s, style: TextStyle(color: c, fontSize: 10, fontWeight: FontWeight.bold)))]));
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)), if (trailing != null) trailing!]),
...
// --- SIDEBAR (હવે મેનૂ/Drawer તરીકે ખુલશે) ---
class SidebarWidget extends StatelessWidget {
  const SidebarWidget({super.key});
  @override
  Widget build(BuildContext context) {
    return Container(width: 280, color: Colors.white, child: Column(children: [const Padding(padding: EdgeInsets.all(24), child: Row(children: [Icon(Icons.star, color: Colors.green), SizedBox(width: 8), Text("Humanline", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))])), _item(Icons.grid_view, "Dashboard", true), _item(Icons.people, "Employees", false), _item(Icons.checklist, "Checklist", false), _item(Icons.access_time, "Time Off", false)]));
  }
  Widget _item(IconData i, String t, bool a) => Container(margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: a ? Colors.green : Colors.transparent, borderRadius: BorderRadius.circular(8)), child: ListTile(dense: true, leading: Icon(i, color: a ? Colors.white : Colors.grey), title: Text(t, style: TextStyle(color: a ? Colors.white : Colors.black87))));
}

// --- TOPBAR (મોબાઈલ માટે મેનૂ બટન અને રિસ્પોન્સિવ સર્ચ) ---
